...
from fastmri.data.transforms import UnetDataTransformChirp
...
def generate_chirp(n):
    ...
    phi= np.asmatrix(phi)
    for i in range(n):
        phi[i,i]=np.exp(-1j*(((delt_C*Tenco*((i+1)**2))/(2*(n**2)))+((1/8)*(delt_C*Tenco))+(np.pi/2))) 
    FFTMTX = (1/np.sqrt(n))*np.fft.fftshift(np.fft.fft(np.fft.ifftshift(np.eye(n))))
    encoder= np.asmatrix(FFTMTX)*phi
...
def cli_main(args):
    ...
    train_transform = UnetDataTransformChirp(args.challenge, mask_func=mask, use_seed=False, chirp_encoder=chirp_encoder, fourier_encoder=fourier_encoder)
    val_transform = UnetDataTransformChirp(args.challenge, mask_func=mask, chirp_encoder=chirp_encoder, fourier_encoder=fourier_encoder)
    test_transform = UnetDataTransformChirp(args.challenge,chirp_encoder=chirp_encoder, fourier_encoder=fourier_encoder)
    ...
    if args.mode == "train":
        ...
        print("\nTotal Time : {:.2f}".format(total_time))
    elif args.mode == "test":
...
def build_args():
    ...
    parser.set_defaults(
        ...
        lr=0.001,  # 0.001 #RMSProp learning rate
